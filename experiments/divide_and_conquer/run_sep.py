        ...
        for d in range(deblend_cont.shape[0]):
            for c in range(clean_param.shape[0]):
                ...
                for i in range(num_images):
                    sep_results = sep.extract((padded_images[i] - background).cpu().numpy(),
                    ...
                    locs[:count,0] = torch.from_numpy(sep_results['y']) - padding + 0.5  # match SMC locs convention
                    locs[:count,1] = torch.from_numpy(sep_results['x']) - padding + 0.5  # match SMC locs convention
                    
                    in_bounds = torch.all(
                        torch.logical_and(locs > 0,
                                          locs < torch.tensor((image_height, image_width))),
                        dim = -1
                    )
...
for i in range(num_images):
    ...
    locs[i,:count,0] = torch.from_numpy(sep_results['y']) - padding + 0.5  # match SMC locs convention
    locs[i,:count,1] = torch.from_numpy(sep_results['x']) - padding + 0.5  # match SMC locs convention
