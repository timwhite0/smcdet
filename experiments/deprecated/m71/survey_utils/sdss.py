...
from experiments.m71.survey_utils.catalog import FullCatalog, SourceType
from experiments.m71.survey_utils.download_utils import download_file_to_dst
from experiments.m71.survey_utils.psf import ImagePSF, PSFConfig
from experiments.m71.survey_utils.survey import Survey

SDSSFields = List[
    TypedDict("SDSSField", {"run": int, "camcol": int, "fields": List[int]})
]
...
class SloanDigitalSkySurvey(Survey):
    ...
    def __init__(
        ...
        self.downloader = SDSSDownloader(
            self.image_ids(), download_dir=str(self.sdss_path)
        )

        self.psf = SDSS_PSF(
            dir_path, self.image_ids(), range(len(self.BANDS)), psf_config
        )
    ...
    def get_from_disk(self, idx):
        ...
        for k in frame_list[0]:
            ...
            item[k] = (
                np.stack(band_data)
                if isinstance(band_data[0], np.ndarray)
                else band_data
            )
...
class PhotoFullCatalog(FullCatalog):
    ...
    def from_file(cls, cat_path, wcs: WCS, height, width):
        ...
        galaxy_fluxes = column_to_tensor(table, "cmodelflux") * galaxy_bools.reshape(
            -1, 1
        )
...
def nelec_to_nmgy_for_catalog(est_cat, nelec_per_nmgy_per_band):
    ...
    nelec_per_nmgy_per_band = torch.tensor(
        nelec_per_nmgy_per_band, device=est_cat.device
    )
