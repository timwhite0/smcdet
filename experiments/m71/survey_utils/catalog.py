...
class BaseTileCatalog(UserDict):
    ...
    def filter_by_ploc_box(self, box_origin: torch.Tensor, box_len: float):
        ...
        plocs_mask = torch.all(
            (self["plocs"] < box_end_tensor) & (self["plocs"] > box_origin_tensor),
            dim=2,
        )

        plocs_mask_indexes = plocs_mask.nonzero()
        plocs_inverse_mask_indexes = (~plocs_mask).nonzero()
        plocs_full_mask_indexes = torch.cat(
            (plocs_mask_indexes, plocs_inverse_mask_indexes), dim=0
        )
        _, index_order = plocs_full_mask_indexes[:, 0].sort(stable=True)
        plocs_full_mask_sorted_indexes = plocs_full_mask_indexes[
            index_order.tolist(), :
        ]
...
class TileCatalog(BaseTileCatalog):
    ...
    def to_full_catalog(self, tile_slen):
        ...
        for param_name, tile_param in tile_params_to_gather.items():
            ...
            indices_for_param = repeat(
                indices_to_retrieve, "b nth_ntw_s -> b nth_ntw_s k", k=k
            )
    ...
    def get_indices_of_on_sources(self) -> Tuple[Tensor, Tensor]:
        ...
        tile_is_on_array = rearrange(
            tile_is_on_array_sampled, "b nth ntw d -> b (nth ntw d)"
        )
    ...
    def get_brightest_sources_per_tile(self, top_k=1, exclude_num=0, band=2):
        ...
        for key, val in sorted_self.items():
            if key == "n_sources":
                d[key] = (sorted_self["n_sources"] - exclude_num).clamp(
                    min=0, max=top_k
                )
            else:
...
class FullCatalog(UserDict):
    ...
    def apply_param_bin(self, pname: str, p_min: float, p_max: float):
        ...
        as_indices = (
            torch.arange(0, max_sources).expand(n_batches, max_sources).unsqueeze(-1)
        )
    ...
    def to_tile_catalog(
        ...
        *,
        ...
        tile_n_sources = torch.zeros(
            tile_cat_shape[:3], dtype=torch.int64, device=self.device
        )
        tile_params = {
            key: torch.zeros(
                (
        ...
        plocs_mask = ((plocs >= plocs_start_point) & (plocs <= plocs_end_point)).all(
            dim=-1
        )
        ...
        assert (
            torch.masked_select(num_shared_tiles_per_source, mask=~plocs_mask) == 0
        ).all()
        # for each source, find how many sources share the same tile with it #
        ...
        if max_shared_tiles > 1:
            source_cum = torch.zeros_like(
                source_to_tile_indices, dtype=inter_int_type
            )  # (b, bm)
            s_to_t_indices_level = torch.cumsum(
                source_to_tile_indices.amax(dim=-1) + 1, dim=0
            )
        ...
        for tile_k, tile_v in tile_params.items():
            ...
            transposed_v = torch.cat(
                (transposed_v, pad), dim=1
            )  # (b (m nth ntw + 1) k)
    ...
    def filter_by_ploc_box(
        self, box_origin: torch.Tensor, box_len: float, exclude_box=False
    ):
        assert box_origin[0] + box_len <= self.height, "invalid box"
        assert box_origin[1] + box_len <= self.width, "invalid box"
