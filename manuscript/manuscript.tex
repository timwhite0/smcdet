% SDSI (Statistics in Data Science and Imaging) template
...
%%% Citations: natbib with SDSI agsm style (author-year)
\usepackage[]{natbib}
...
\urlstyle{same}
\hypersetup{
  pdftitle={Sequential Monte Carlo Samplers for Probabilistic Object Detection in Images},
  pdfauthor={Tim White; Jeffrey Regier},
  pdfkeywords={interacting particle systems, marked point processes, posterior sampling, likelihood tempering, transdimensional inference, Bayesian model choice},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
%%% SDSI anonymization flag
%%% Set to 0 to hide authors and acknowledgements (for anonymous review)
%%% Set to 1 to show authors and acknowledgements
\newcommand{\anon}{1}
...
\if1\anon
{
  \title{\bf Sequential Monte Carlo Samplers for Probabilistic Cataloging of Crowded Starfields}
  \author{Tim White\thanks{
    This manuscript is based upon work supported by the National Science Foundation (\texttt{<grant identification number omitted>}).}\hspace{.2cm}\\
...
\if0\anon
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf Sequential Monte Carlo Samplers for Probabilistic Cataloging of Crowded Starfields}
  \end{center}
  \medskip
} \fi
...
Code to reproduce both sets of experiments is publicly available at \url{https://github.com/timwhite0/smc_object_detection}.
...
\begin{description}
\item[Appendix A:] Additional results for synthetic images. (PDF)
\item[Appendix B:] Additional results for the M71 globular cluster. (PDF)
\end{description}
