...
class MetropolisHastings(object):
    ...
    def run(self, counts, locs, features, temperature, log_target):
        ...
        for iter in range(self.num_iters):
            locs_proposed = TruncatedDiagonalMVN(locs_prev, self.locs_stdev, self.locs_min,
                                                 self.locs_max).sample() * count_indicator.unsqueeze(4)
            features_proposed = TruncatedDiagonalMVN(features_prev, self.features_stdev, self.features_min,
                                                     self.features_max).sample() * count_indicator
